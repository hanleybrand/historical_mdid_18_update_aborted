...
from rooibos.access import filter_by_access, get_effective_permissions_and_restrictions, get_accesscontrols_for_object, \
    check_access
...
from rooibos.storage import get_media_for_record, get_image_for_record, get_thumbnail_for_record, match_up_media, \
    analyze_media, analyze_records, find_record_by_identifier
...
def formtest(request):
    storage_fs = StorageForm
    if request.method == 'POST':
        formset = storage_fs(request.POST, request.FILES)
        if formset.is_valid():
            formset.save()
            # do something.
    else:
